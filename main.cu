...
__device__ vect rayColor(const ray &r, hitable **world,
  ...
  return vect(0.0, 0.0, 0.0); // exceeded recursion
}
...
__global__ void render(vect *fb, int max_x, int max_y, int ns, camera **cam,
...
__global__ void create_world(hitable **d_list, hitable **d_world,
  ...
  if (threadIdx.x == 0 && blockIdx.x == 0) {
    ...
    d_list[0] = new sphere(vect(0, -1000.0, -1), 1000,
                           new lambertian(vect(0.5, 0.5, 0.5)));
    int i = 1;
    for (int a = -11; a < 11; a++) {
      for (int b = -11; b < 11; b++) {
        float choose_mat = RND;
        vect center(a + RND, 0.2, b + RND);
        if (choose_mat < 0.8f) {
          d_list[i++] =
              new sphere(center, 0.2,
                         new lambertian(vect(RND * RND, RND * RND, RND * RND)));
        } else if (choose_mat < 0.95f) {
          d_list[i++] = new sphere(
              center, 0.2,
              new metal(vect(0.5f * (1.0f + RND), 0.5f * (1.0f + RND),
                             0.5f * (1.0f + RND)),
                        0.5f * RND));
        } else {
    ...
    d_list[i++] = new sphere(vect(0, 1, 0), 1.0, new dielectric(1.5));
    d_list[i++] =
        new sphere(vect(-4, 1, 0), 1.0, new lambertian(vect(0.4, 0.2, 0.1)));
    d_list[i++] =
        new sphere(vect(4, 1, 0), 1.0, new metal(vect(0.7, 0.6, 0.5), 0.0));
    *rand_state = local_rand_state;
    *d_world = new hitable_list(d_list, 22 * 22 + 1 + 3);

    vect lookfrom(13, 2, 3);
    vect lookat(0, 0, 0);
    float dist_to_focus = 10.0;
    (lookfrom - lookat).len();
    float aperture = 0.1;
    *d_camera = new camera(lookfrom, lookat, vect(0, 1, 0), 30.0,
